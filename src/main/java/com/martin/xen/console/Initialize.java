...
public class Initialize extends JApplet implements ConnectionListener, ConsoleListener {
	...
	public void init() {
		try {
			...
			if (getParameter("BACKCOLOR") != null) {
				...
				String[] vals = s.split(":");
				if (vals.length == 3 && vals[0].length() == 2 && vals[1].length() == 2
						&& vals[2].length() == 2) {
					c = new Color(Integer.parseInt(vals[0], 16), Integer.parseInt(vals[1], 16),

...
public class VNCCanvas extends JComponent implements VNCClient, ClipboardOwner, MouseListener,
	...
	public VNCCanvas() {
		...
		BufferedImage blankThumbnail = new BufferedImage(200, 150, BufferedImage.TYPE_INT_RGB);
	...
	public Dimension getPreferredSize() {
		if (image_ != null) {
			return new Dimension(Translate(_streamWidth), Translate(_streamHeight));
		}
	...
	public void paintComponent(Graphics graphics) {
		...
		if (image_ != null && ui == null) {
			...
			if (!isFullscreen) {
				...
				g2.drawImage(image_, 0, 0, Translate(_streamWidth), Translate(_streamHeight), null);
			}
		}
	}
	...
	private void pointerEvent(MouseEvent event) {
		if (this.hasFocus()) {
			...
			stream_.pointerEvent(buttonMask, TranslateRev(event.getX()), TranslateRev(event.getY()));
			// event.consume();
		}
	}
		...
		if (this.hasFocus()) {
			if (_cursor != null) {
				...
				Image image = Toolkit.getDefaultToolkit().createImage(
						new java.awt.image.MemoryImageSource(16, 16, pixels, 0, 16));
	...
	private void key(KeyEvent event, boolean pressed) {
		if (this.hasFocus()) {
			if (event.getKeyCode() == KeyEvent.VK_ALT && event.isControlDown() && isFullscreen) {
				...
				isFullscreen = false;
				// screen = null;
			} else if (event.getKeyCode() == KeyEvent.VK_ALT && event.isControlDown()) {
				/*
				 * For now disabling Ctrl-Alt fullscreen support final Container
				 * root = getParent().getParent(); root.remove(getParent());
				 * VNCFullscreen fc = new VNCFullscreen((JPanel) getParent(),
				 * this, true, getParent().getBackground());
				 * fc.addWindowListener(new WindowListener() { public void
				 * windowActivated(WindowEvent e) { };
				 * 
				 * public void windowClosed(WindowEvent e) {
				 * root.add(getParent()); setMaxHeight(getParent().getHeight());
				 * setMaxWidth(getParent().getWidth()); root.invalidate();
				 * root.validate(); invalidate(); repaint();
	...
	private void damage(int x, int y, int width, int height) {
		...
		if (damageStreamside.isEmpty()) {
			...
			damageStreamside.add(new Rectangle(Translate(x), Translate(y), Translate(width),
					Translate(height)));
		}
	}
	 ...
	 */
	public void clientDrawImage(final Image image, final int x, final int y, final int width,
			final int height) {
	...
	public void clientSetCursor(Image image, int x, int y) {
		...
		Dimension cursorSize = toolkit.getBestCursorSize(imageWidth, imageHeight);

		if (!(imageWidth > cursorSize.width || imageHeight > cursorSize.height)
				&& (imageWidth < cursorSize.width || imageHeight < cursorSize.height)) {
			BufferedImage bi = new BufferedImage(cursorSize.width, cursorSize.height,
		...
		final Cursor cursor = toolkit.createCustomCursor(image, new Point(x, y), "");
	 ...
	 */
	public void clientCopyRectangle(final int x, final int y, final int width, final int height,
	 ...
	 */
	public void clientFillRectangle(final int x, final int y, final int width, final int height,
			final Color color) {
